...
func (r *BrandPsqlRepository) CreateBrandModel(ctx context.Context, model models.BrandModel) (int64, error) {
	...
	query := ` INSERT INTO brand_models (name, logo_path, brand_id) VALUES ($1, $2, $3) RETURNING id `

	err := r.client.QueryRow(ctx, query, model.Name, model.LogoPath, model.BrandID).Scan(&id)
	if err != nil {
...
func (r *BrandPsqlRepository) GetBrandModels(ctx context.Context, limit, page int64, search string) ([]models.BrandModel, int64, error) {
	...
	query := `
		SELECT 
		    bm.id, bm.name, bm.logo_path, bm.brand_id, b.name 
...
func (r *BrandPsqlRepository) UpdateBrandModel(ctx context.Context, model models.BrandModel) (int64, error) {
	...
	query := `
		UPDATE brand_models SET 
		    name = $1, logo_path = $2, brand_id = $3, updated_at = NOW()
		WHERE id = $4
		RETURNING id;
	`
	err := r.client.QueryRow(ctx, query, model.Name, model.LogoPath, model.BrandID, model.ID).Scan(&id)
	if err != nil {
...
func (r *BrandPsqlRepository) GetBrandModelByID(ctx context.Context, id int64) (models.BrandModel, error) {
	var model models.BrandModel
	...
	query := `
		...
		WHERE bm.id = $1
	`
	err := r.client.QueryRow(ctx, query, id).Scan(&model.ID, &model.Name, &model.LogoPath, &model.BrandID, &model.BrandName)
	if err != nil {
		r.logger.Errorf("get brand model by id query err : %v", err)
		return model, err
	}
	return model, nil
}
